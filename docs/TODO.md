...
- **ZOSC 字标动效预览（2026-09-22）**：原型只在 `preview/zosc-logo-motion` 分支的 `/preview/zosc-intro/` 独立页，生产 `main` 与 `zerb.net` 首页不改。待用户看预览后确认字母变形、心跳节奏，以及是否将首页的 `Motion / Visual / Code` 大标题改为更宽泛的品牌表达；确认前不做生产迁移。
...
## 四字母 .com 真实行情校准（2026-09-11，用户实测数据）

**唯一可信的价格锚点，来自用户自己查到的同场成交（DSAD / DNScroll 两边一致）：**

| 域名 | 成交 | 出价次数 | 可发音？ |
|---|---|---|---|
| DYII.com | **$310** | 45 | ❌ 双 i 结尾，念不出 |
| AOLD.com | **$1,025** | 50 | ✅ 读作 old / auld |
| zosc.com | ~$1,000（2026-09-11 进行中） | — | ✅ 读作 zosk |
| zezr.com | ¥201.68 时被原主撤拍 | 25 | ❌ `/zr/` 不合法 |

**结论：可发音 vs 不可发音，差约 3.3 倍。可发音四字母 .com 的真实行情 ≈ $1,000（¥7,000）。**

### ⚠️ 中途价格没有参考价值（2026-09-11 用户纠正）

```
zosc.com   9/07 ¥464(≈$65) 剩 4 天  →  9/11 ~$1,000     最后阶段涨约 15 倍
```

原因：代理出价的上限要到末段才被逐个试穿，职业买家与狙击手也压在最后进场；
**前面大半程的低价只说明还没人认真出手。**

因此：
- `zezr.com` 的 ¥201.68 是**开赛第 10 天**的价（22 天赛程），被撤时全程未走完，
  **它的最终价不可知**——不能据此说"zezr 比 zosc 便宜"。（我原先这么推过，错了。）
- **看到中途价，直接按 10–15 倍估终局。**
- **天花板必须从「可比成交」推**（DSAD / DNScroll 查同类同期落槌价），**不能从当前价推**。
  我当初看到 ¥464、两个竞标者在 ¥379 停住就判断"¥500–700 能拿下"——那是拿早期冷清当终局。


### ⚠️ 我（Claude）在这件事上犯的两个错，记下来防止重犯

1. **分类错误**：把 zosc 归进 "$150–250 批发档"。我引用的那个 $192/个批发包里是
   `KQVY`/`KVXG`/`KZUQ` 这种**念不出来的字母垃圾**，而 zosc **能念**——可发音四字母是独立的
   brandable 档，行情高一个数量级。**拿垃圾档价格套 brandable，是分类错误不是数据误差。**
2. **明知置信区间宽还给点预测**：NameBio 进不去（Cloudflare 403）我是知道的，却仍predicted
   "落槌 ¥500–700"。实际 ~$1,000。**取不到权威数据时应该只给区间和依据，不给点预测。**
3. **搜索停得太早**：试了 namebio + dsad 某页各一次 403 就下结论"查不到"，**dnscroll 一次都没试**。
   用户用 DSAD + DNScroll 查到了。**403 不等于没有，换源再试。**

**给未来的规矩**：给这一档估价前，先去 DSAD / DNScroll 查同期同类实际成交，不要引用论坛挂单价。
