...
## 2026-09-23 ZOSC 动效第二版

- 用户否定第一版后授权自由重做：独立预览页改为自托管 Montserrat 900 小写 `zosc.`，暖白字标配橙色句点，移除线框字形、外框与逐步解释字幕。
- 入场改为圆点起势、字母从中心展开、圆点沿弧线落到句尾并回弹；支持重播、减少动效、字体加载后测量，以及窗口尺寸变化时回到完整字标。
- 真实 Chromium 检查桌面、390px 手机、重播与减少动效；字标完整、无横向溢出。仍限定在原预览分支，不进入生产。

## 2026-09-22 ZOSC 字标动效预览
